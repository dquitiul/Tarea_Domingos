...
def empAdministrativos():
    borrarPantalla()     
    gotoxy(20,2);print("MANTENIMIENTO DE EMPLEADO ADMINISTRATIVO")
    empleado,entEmpleado = [],None

    #nombre,departamento,cargo,direccion,cedula,telefono,fechaIngreso,sueldo=0,0,0,0,0,0,0,0
    cedula,telefono,sueldo=0,0,0
    while not empleado:
    ...
    # gotoxy(15,6);print("nombre:")
    # gotoxy(15,7);print("departamento:")
    # gotoxy(15,8);print("cargo:")
    # gotoxy(15,9);print("direccion:")
      gotoxy(15,6);print("cedula:")
      gotoxy(15,7);print("telefono:")
    # gotoxy(15,12);print("fechaIngreso:")
      gotoxy(15,8);print("sueldo:")

    validar = Valida()
    cedula=validar.solo_numeros("Error: Solo numeros",23,6)
   #gotoxy(23,6);aamm = input()
    # gotoxy(23,7);departamento = input()
    # gotoxy(24,8);cargo = input()
    # gotoxy(25,9);direccion = input()
    #gotoxy(26,6);cedula = input()
    gotoxy(24,7);telefono = input()
    # gotoxy(28,12);fechaIngreso = input()
    gotoxy(25,8);sueldo = input()

    gotoxy(15,9);print("Esta seguro de Grabar El registro(s/n):")
    gotoxy(54,9);grabar = input().lower()
    if grabar == "s":
        archiAdministrativo = Archivo("./archivos/administrativo.txt","|")
        administrativos = archiAdministrativo.leer()
        if administrativos : idSig = int(administrativos[-1][0])+1
        else: idSig=1
        #administrativo = Administrativo(entEmpleado,departamento,cargo,direccion,cedula,telefono,fechaIngreso,sueldo,True,idSig)
        administrativo = Administrativo(entEmpleado,cedula,telefono,sueldo,True,idSig)
        datos = administrativo.getEmpleadoA()
        datos = '|'.join(datos)
        archiAdministrativo.escribir([datos],"a")                 
    ...
    if grabar == "s":
        ...
        if ListaEmpObr :
            ...
            for empleado in ListaEmpObr:
              #print(empleado)
              entEmpObr = Obrero(empleado[1],empleado[2],empleado[3],empleado[4],empleado[5],empleado[6],empleado[7],float(empleado[8]),empleado[0]) 
              #print(entEmpAdm.nombre,entEmpAdm.sueldo)
